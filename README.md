...
- **문제은행 관리**:
  - 버전 관리와 함께 문제 세트 가져오기, 검증 및 관리
  - 여러 JSON 파일 동시 import 및 JSON 직접 붙여넣기 지원
  - Import 진행률 실시간 표시
  - `mergeByMeta=true` 옵션으로 동일 title/description 세트에 누적 import
  - merge import 시 이미 존재하는 문제 ID는 자동 건너뜀
- **연습 모드**:
  - 전체 문제 세트로 정규 시험 연습
  - 커스텀 트레이닝 모드 (주제/난이도 필터링)
  - 주제 선택 드롭다운 (체크박스로 다중 선택)
  - 조기 종료 버튼 ("결과 보기")
  - 결과 페이지에서 틀린 문제 해설 자동 표시
- **오답노트 시스템**:
  - 틀린 문제들을 오답노트에 저장
  - 저장된 문제만 다시 풀기
  - 2번 연속 정답 시 자동 마스터 처리
  - 모든 문제 마스터 시 자동 아카이브
  - 활성/보관된 오답노트 구분 관리
  - 보관된 오답노트도 "다시 풀기"로 재도전 가능
- **진행 상황 추적**: 세트, 주제, 난이도별 점수 추적
- **70% 목표 지원**: 약점 영역에 대한 명확한 가이드와 추천 연습
- **보안**: `ADMIN_PASSWORD` 기반 관리자 인증 (`ADMIN_KEY` 하위 호환)
- **유효성 검사**: Zod
...
3. **환경 구성**
  ...
  - environment 섹션에서 `ADMIN_PASSWORD` 설정
...
### 오답노트
- `POST /api/mistakes` - 오답노트 생성
- `GET /api/mistakes` - 오답노트 목록 (includeArchived 옵션)
- `GET /api/mistakes/:id` - 특정 오답노트 조회
- `POST /api/mistakes/:id/update` - correctStreak 업데이트 (자동 아카이빙)
- `DELETE /api/mistakes/:id` - 오답노트 소프트 삭제

### 문제 조회
- `POST /api/questions` - questionIds로 문제 조회 (오답노트용)

## 핵심 기능

### 가져오기 시스템
- **유연한 형식**: 완전한 세트와 문제만 있는 배열 모두 지원
- **다중 파일 import**: 여러 JSON 파일을 한 번에 선택하여 업로드
- **진행률 표시**: 실시간 import 진행 상황 (N/M 카운터 및 진행 바)
- **유효성 검사**: 상세한 오류 메시지와 함께 포괄적인 스키마 검증
- **미리보기**: 문제 수, 주제 분포, 난이도 분석 표시
- **중복 감지**: 중복 문제 ID 방지
- **중복 처리**: 기본 import에서는 중복 ID를 오류로 반환하고, `mergeByMeta=true`에서는 이미 존재하는 ID를 자동 건너뜀
- **자동 ID 생성**: ID가 없는 문제에 자동으로 UUID 할당
...
- 문제 네비게이션이 있는 진행 표시줄
- 특정 문제 번호로 점프
- **조기 종료 버튼**: 마지막 문제까지 풀지 않아도 "결과 보기" 가능
- 클립보드에 문제 복사 (ChatGPT 호환 형식)
- 자동 저장 진행 상황 (재로드 시 이어하기)

### 커스텀 트레이닝
- **주제 선택 드롭다운**: 체크박스로 여러 주제 동시 선택
- "전체 선택" 옵션으로 빠른 선택
- 난이도별 필터링 (쉬움/보통/어려움)
- 문제 수 설정 (5-50문제)
- 무작위 문제 선택
- 주제/난이도별 결과 분석
- **조기 종료 버튼**: 언제든지 결과 확인 가능

### 오답노트
- **결과 페이지 기능**:
  - 틀린 문제 모두 표시 (정답/오답/해설 포함)
  - "오답노트에 저장" 버튼으로 수동 저장
  - 저장 전 확인 다이얼로그
- **오답노트 페이지**:
  - 활성 오답노트: 남은 문제 개수, 마스터 진행률 표시
  - 보관된 오답노트: 완료/삭제된 항목 (토글로 보기/숨기기)
- **오답 연습 모드**:
  - 저장된 틀린 문제만 다시 풀기
  - 각 문제마다 현재 연속 정답 횟수 표시
  - 2번 연속 정답 시 자동 마스터 처리
  - 완료 시 진행 상황 자동 업데이트
- **스마트 아카이빙**:
  - 모든 문제 마스터 시 자동 아카이브
...
- Difficulty: 1, 2, 또는 3
- Stem: 필수, 비어있지 않은 문자열
- Choices: 2~6개의 비어있지 않은 문자열
- Answer: 0-3 사이의 숫자
- Explanation: 선택 (빈 문자열 허용)
...
# 보안
ADMIN_PASSWORD=your-secret-password   # 관리자 로그인 비밀번호 (권장)
ADMIN_KEY=your-secret-key             # 하위 호환용 (선택)
                                      # 관리자 페이지 접근 시 한 번만 입력
