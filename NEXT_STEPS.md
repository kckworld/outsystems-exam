...
3. 아래 시나리오 수동 점검
4. `맞춤학습 세트 선택 시 토픽도 해당 세트 기준으로 제한` 작업 착수

## 내일 1순위 추가 작업

- 요구사항:
  - 맞춤학습에서 문제 세트를 선택하면 토픽 목록도 그 세트에 포함된 토픽만 표시
  - 문제 세트를 `전체`로 바꾸면 전체 토픽 다시 표시
- 구현 가이드:
  1. `GET /api/topics`에 `setId` 쿼리 파라미터 지원
  2. `app/(train)/train/page.tsx`에서 `selectedSetId` 변경 시 토픽 재조회
  3. 세트 변경 시 `selectedTopics`를 새 토픽 목록 기준으로 재정렬(기본 전체 선택)
  4. 토픽 0개인 세트의 안내 문구 처리
- 영향 파일(예정):
